        ...
        nodeContent={
          <fieldset disabled={blnReadOnly || blnSaving} style={{ border: 0, padding: 0, margin: 0 }}>
            <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "repeat(2, 1fr)" }, gap: 1.5 }}>
              ...
              <FormControlLabel control={<Switch checked={objForm.blnIsPaid} onChange={(objEvent) => setObjForm((objPrevious) => ({ ...objPrevious, blnIsPaid: objEvent.target.checked }))} />} label={t("field_paid", "Paid")} />
              <FormControlLabel control={<Switch checked={objForm.blnIsOptional} onChange={(objEvent) => setObjForm((objPrevious) => ({ ...objPrevious, blnIsOptional: objEvent.target.checked }))} />} label={t("field_optional", "Optional")} />
              <FormControlLabel control={<Switch checked={objForm.blnWorkOnHolidayAllowed} onChange={(objEvent) => setObjForm((objPrevious) => ({ ...objPrevious, blnWorkOnHolidayAllowed: objEvent.target.checked, blnCompOffEligible: objEvent.target.checked ? objPrevious.blnCompOffEligible : false }))} />} label={t("field_work_allowed", "Work on Holiday Allowed")} />
              <FormControlLabel control={<Switch checked={objForm.blnCompOffEligible} disabled={blnReadOnly || !objForm.blnWorkOnHolidayAllowed} onChange={(objEvent) => setObjForm((objPrevious) => ({ ...objPrevious, blnCompOffEligible: objEvent.target.checked }))} />} label={t("field_comp_off", "Comp-Off Eligible")} />
              <Stack direction="row" alignItems="center" spacing={1}><ActiveStatusSwitch blnIsActive={objForm.blnIsActive} disabled={blnReadOnly} onChange={(blnIsActive) => setObjForm((objPrevious) => ({ ...objPrevious, blnIsActive }))} /><Typography>{t("field_is_active", "Active")}</Typography></Stack>
              <Box sx={{ gridColumn: { sm: "span 2" }, borderTop: "1px solid #e2e8f0", pt: 1.5 }}>
                <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 1 }}><Typography sx={{ fontWeight: 800 }}>{t("multilingual_text", "Multilingual Text")}</Typography>{!blnReadOnly && objOptions.intSecondaryLanguageID ? <Button size="small" startIcon={<AutoAwesomeRoundedIcon />} disabled={blnTranslating || !objForm.strHolidayName.trim()} onClick={() => void translateSecondaryName()}>{blnTranslating ? t("translating", "Translating...") : t("translate", "AI Translate")}</Button> : null}</Stack>
                <Stack spacing={1}>
                  {objForm.lstTexts.map((objText, intIndex) => <Stack key={objText.intLanguageID} direction={{ xs: "column", sm: "row" }} spacing={1}><TextField size="small" label={t("language", "Language")} value={objOptions.lstLanguages.find((objLanguage) => objLanguage.intID === objText.intLanguageID)?.strLabel ?? String(objText.intLanguageID)} disabled sx={{ width: { sm: 180 } }} /><TextField size="small" fullWidth label={t("field_name", "Name")} value={objText.strHolidayName} onChange={(objEvent) => { const strName = objEvent.target.value; setObjForm((objPrevious) => ({ ...objPrevious, strHolidayName: objText.intLanguageID === objOptions.intDefaultLanguageID ? strName : objPrevious.strHolidayName, lstTexts: objPrevious.lstTexts.map((objRow, intRowIndex) => intRowIndex === intIndex ? { ...objRow, strHolidayName: strName } : objRow) })); }} /></Stack>)}
                </Stack>
              </Box>
            </Box>
          </fieldset>
