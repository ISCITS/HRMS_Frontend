...
}).test("thresholds", "Threshold order must be Full Day ≥ Half Day ≥ Absent.", function (objValue) {
    if (!objValue || (objValue.decFullDayThresholdHours >= objValue.decHalfDayThresholdHours && objValue.decHalfDayThresholdHours >= objValue.decAbsentThresholdHours)) return true;
    return this.createError({ path: "decHalfDayThresholdHours" });
  })
  .test("dates", "Effective To cannot be before Effective From.", function (objValue) {
    if (!objValue?.dtEffectiveTo || objValue.dtEffectiveTo >= objValue.dtEffectiveFrom) return true;
    return this.createError({ path: "dtEffectiveTo" });
  })
  .test("in-out-time", "Out Time must be later than In Time.", function (objValue) {
    if (!objValue?.strInTime || !objValue?.strOutTime || objValue.strOutTime > objValue.strInTime) return true;
    return this.createError({ path: "strOutTime" });
  })
...
export default function AttendancePocPanel({ strView }: AttendancePocPanelProps) {
  ...
  const nodePolicyForm = <Grid component="fieldset" disabled={blnPolicyViewMode} container spacing={1.5} sx={{
    ...
    <Grid item xs={12} lg={6}><Paper variant="outlined" sx={{ p: 1.5, height: "100%" }}><Typography fontWeight={800} sx={{ mb: 1 }}>{t("working_hour_rules", "Working Hour Rules")}</Typography><Grid container spacing={1}>
      <Grid item xs={6} sm={4}><Controller name="strInTime" control={control} render={({ field }) => <TextField {...field} value={field.value ?? ""} data-control-id="attendance.policy.in-time.input" label={t("in_time", "Standard IN Time")} type="time" InputLabelProps={{ shrink: true }} fullWidth error={!!errors.strOutTime} />} /></Grid>
      <Grid item xs={6} sm={4}><Controller name="strOutTime" control={control} render={({ field }) => <TextField {...field} value={field.value ?? ""} data-control-id="attendance.policy.out-time.input" label={t("out_time", "Standard OUT Time")} type="time" InputLabelProps={{ shrink: true }} fullWidth error={!!errors.strOutTime} helperText={errors.strOutTime?.message} />} /></Grid>
      {([["decFullDayThresholdHours","full_day_threshold","Full-Day Threshold"],["decHalfDayThresholdHours","half_day_threshold","Half-Day Threshold"]] as const).map(([strName,strKey,strLabel]) => <Grid item xs={6} sm={4} key={strName}><Controller name={strName} control={control} render={({ field }) => <TextField {...field} data-control-id={`attendance.policy.${strName}.input`} label={t(strKey,strLabel)} type="number" fullWidth error={!!errors[strName] || !!errors.decHalfDayThresholdHours} helperText={strName === "decHalfDayThresholdHours" ? errors.decHalfDayThresholdHours?.message : undefined} inputProps={{ min: 0, step: .25 }} />} /></Grid>)}
    ...
    <Grid item xs={12} lg={6}><Paper variant="outlined" sx={{ p: 1.5, height: "100%" }}><Typography fontWeight={800} sx={{ mb: 1 }}>{t("weekly_off_overtime", "Weekly Off and Overtime")}</Typography><Grid container spacing={1}>
      ...
      <Grid item xs={12} md={4}><Controller name="decOtMinHours" control={control} render={({ field }) => <TextField {...field} data-control-id="attendance.policy.ot-min.input" disabled={!blnOtEnabled} label={t("minimum_overtime", "OT Hours per Comp-Off Day")} type="number" fullWidth />} /></Grid>
