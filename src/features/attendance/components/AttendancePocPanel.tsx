...
export default function AttendancePocPanel({ strView }: AttendancePocPanelProps) {
  ...
  const nodePolicyForm = <Grid component="fieldset" disabled={blnPolicyViewMode} container spacing={1.5} sx={{
    ...
    <Grid item xs={12} lg={6}><Paper variant="outlined" sx={{ p: 1.5, height: "100%" }}><Typography fontWeight={800} sx={{ mb: 1 }}>{t("general_information", "General Information")}</Typography><Grid container spacing={1}>
      ...
      <Grid item xs={12}>
        <Alert severity="info" data-control-id="attendance.policy.punch-rule.note" sx={{ py: 0.75 }}>{t("punch_rule_note", "Both IN and OUT punches are required on a normal working day. If both punches are missing after attendance finalization, the day is marked Absent. If exactly one punch is missing, an Attendance Exception is created for regularization.")}</Alert>
    ...
    <Grid item xs={12} lg={6}><Paper variant="outlined" sx={{ p: 1.5, height: "100%" }}><Typography fontWeight={800} sx={{ mb: 1 }}>{t("working_hour_rules", "Working Hour Rules")}</Typography><Grid container spacing={1}>
      ...
      <Grid item xs={12} sm={4}><Controller name="intLateGraceMinutes" control={control} render={({ field }) => <TextField {...field} data-control-id="attendance.policy.late-grace.input" label={t("late_grace", "Late Arrival Grace (Minutes)")} type="number" fullWidth />} /></Grid>
      <Grid item xs={12} sm={4}><Controller name="intEarlyDepartureGraceMinutes" control={control} render={({ field }) => <TextField {...field} data-control-id="attendance.policy.early-grace.input" label={t("early_grace", "Early Departure Grace (Minutes)")} type="number" fullWidth />} /></Grid>
      <Grid item xs={12} sm={4}><Controller name="intWorkHoursRoundingMinutes" control={control} render={({ field }) => <TextField {...field} data-control-id="attendance.policy.rounding.input" label={t("rounding", "Work-Hour Rounding (Minutes)")} type="number" fullWidth />} /></Grid>
