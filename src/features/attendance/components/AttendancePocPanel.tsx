...
export default function AttendancePocPanel({ strView }: AttendancePocPanelProps) {
  ...
  const nodePolicyForm = <Grid component="fieldset" disabled={blnPolicyViewMode} container spacing={1.5} sx={{
    ...
    <Grid item xs={12} lg={6}><Paper variant="outlined" sx={{ p: 1.5, height: "100%" }}><Typography fontWeight={800} sx={{ mb: 1 }}>{t("working_hour_rules", "Working Hour Rules")}</Typography><Grid container spacing={1}>{([["decFullDayThresholdHours","full_day_threshold","Full-day Threshold"],["decHalfDayThresholdHours","half_day_threshold","Half-day Threshold"],["decAbsentThresholdHours","absent_threshold","Absent Threshold"]] as const).map(([strName,strKey,strLabel]) => <Grid item xs={12} md={4} key={strName}><Controller name={strName} control={control} render={({ field }) => <TextField {...field} data-control-id={`attendance.policy.${strName}.input`} label={t(strKey,strLabel)} type="number" fullWidth error={!!errors[strName]} inputProps={{ min: 0, step: .25 }} />} /></Grid>)}</Grid>{errors.root?.message ? <Alert severity="error" sx={{ mt: 1 }}>{errors.root.message}</Alert> : null}</Paper></Grid>
    <Grid item xs={12} lg={7}><Paper variant="outlined" sx={{ p: 1.5, height: "100%" }}><Typography fontWeight={800} sx={{ mb: 1 }}>{t("punch_exception_rules", "Punch and Exception Rules")}</Typography><Grid container spacing={1}>
      <Grid item xs={6} md={3}><Controller name="blnInPunchRequired" control={control} render={({ field }) => <FormControlLabel control={<Switch data-control-id="attendance.policy.in-required.switch" checked={field.value} onChange={field.onChange} />} label={t("in_required", "IN Required")} />} /></Grid>
      <Grid item xs={6} md={3}><Controller name="blnOutPunchRequired" control={control} render={({ field }) => <FormControlLabel control={<Switch data-control-id="attendance.policy.out-required.switch" checked={field.value} onChange={field.onChange} />} label={t("out_required", "OUT Required")} />} /></Grid>
      <Grid item xs={12} md={3}><Controller name="intLateGraceMinutes" control={control} render={({ field }) => <TextField {...field} data-control-id="attendance.policy.late-grace.input" label={t("late_grace", "Late Grace (min)")} type="number" fullWidth />} /></Grid>
      <Grid item xs={12} md={3}><Controller name="intEarlyDepartureGraceMinutes" control={control} render={({ field }) => <TextField {...field} data-control-id="attendance.policy.early-grace.input" label={t("early_grace", "Early Departure Grace")} type="number" fullWidth />} /></Grid>
      <Grid item xs={12} md={7}><Controller name="strMissingPunchTreatmentCode" control={control} render={({ field }) => <TextField {...field} data-control-id="attendance.policy.missing-punch.select" select label={t("missing_punch", "Missing-punch Treatment")} fullWidth>{([["EXCEPTION","Creates attendance exception"],["ABSENT","Mark as Absent"],["HALF_DAY","Mark as Half Day"],["IGNORE","Ignore"]] as const).map(([strValue, strFallback]) => <MenuItem key={strValue} value={strValue}>{t(`missing_${strValue.toLowerCase()}`, strFallback)}</MenuItem>)}</TextField>} /></Grid>
      <Grid item xs={12} md={5}><Controller name="intWorkHoursRoundingMinutes" control={control} render={({ field }) => <TextField {...field} data-control-id="attendance.policy.rounding.input" label={t("rounding", "Work-hour Rounding (min)")} type="number" fullWidth />} /></Grid>
    </Grid></Paper></Grid>
    <Grid item xs={12} lg={5}><Paper variant="outlined" sx={{ p: 1.5, height: "100%" }}><Typography fontWeight={800} sx={{ mb: 0.5 }}>{t("weekly_off_overtime", "Weekly Off and Overtime")}</Typography><Stack direction="row" spacing={1} alignItems="center"><Controller name="blnOtEnabled" control={control} render={({ field }) => <FormControlLabel control={<Switch data-control-id="attendance.policy.ot.switch" checked={field.value} onChange={field.onChange} />} label={t("overtime_enabled", "Overtime")} />} /><Controller name="decOtMinHours" control={control} render={({ field }) => <TextField {...field} data-control-id="attendance.policy.ot-min.input" disabled={!blnOtEnabled} label={t("minimum_overtime", "Minimum OT Hours")} type="number" size="small" sx={{ maxWidth: 170 }} />} /></Stack><Stack direction="row" flexWrap="wrap" sx={{ mt: 0.5 }}>{lstWeekdays.map((strDay,intIndex) => <FormControlLabel sx={{ mr: 1 }} key={strDay} control={<Checkbox size="small" data-control-id={`attendance.policy.weekly-off.${intIndex}.checkbox`} checked={strPattern[intIndex] === "1"} onChange={(objEvent) => { const lstPattern = strPattern.split(""); lstPattern[intIndex] = objEvent.target.checked ? "1" : "0"; setValue("strWeeklyOffPattern", lstPattern.join(""), { shouldDirty: true }); }} />} label={t(`weekday_${strDay.toLowerCase()}`,strDay)} />)}</Stack><Typography variant="caption" color="text.secondary">{t("weekly_off_preview", "Weekly off")}: {lstWeekdays.filter((_,intIndex) => strPattern[intIndex] === "1").join(", ") || t("none", "None")}</Typography></Paper></Grid>
    <Grid item xs={12}><Paper variant="outlined" sx={{ p: 1.5 }}><Typography fontWeight={800} sx={{ mb: 1 }}>{t("effective_dates_status", "Effective Dates and Status")}</Typography><Grid container spacing={1} alignItems="center"><Grid item xs={12} md={2}><Controller name="dtEffectiveFrom" control={control} render={({ field }) => <TextField {...field} data-control-id="attendance.policy.effective-from.input" type="date" label={t("effective_from", "Effective From")} InputLabelProps={{ shrink: true }} fullWidth />} /></Grid><Grid item xs={12} md={2}><Controller name="dtEffectiveTo" control={control} render={({ field }) => <TextField {...field} value={field.value ?? ""} data-control-id="attendance.policy.effective-to.input" type="date" label={t("effective_to", "Effective To")} InputLabelProps={{ shrink: true }} fullWidth />} /></Grid><Grid item xs={12} md={3}><Stack direction="row"><Controller name="blnIsDefault" control={control} render={({ field }) => <FormControlLabel control={<Switch data-control-id="attendance.policy.default.switch" checked={field.value} onChange={field.onChange} />} label={t("default_policy", "Default")} />} /><Controller name="blnIsActive" control={control} render={({ field }) => <FormControlLabel control={<Switch data-control-id="attendance.policy.active.switch" checked={field.value} onChange={field.onChange} />} label={t("active", "Active")} />} /></Stack></Grid><Grid item xs={12} md={5}><Controller name="strRemarks" control={control} render={({ field }) => <TextField {...field} value={field.value ?? ""} data-control-id="attendance.policy.remarks.input" label={t("remarks", "Remarks")} fullWidth />} /></Grid></Grid></Paper></Grid>
      ...
      <Box className={styles.controlsCard}><Box className={styles.searchRow} sx={{ gridTemplateColumns: "165px minmax(190px,.65fr) minmax(190px,.65fr) minmax(260px,1.3fr) auto auto !important" }}><TextField data-control-id="attendance.daily.date.input" type="date" InputLabelProps={{ shrink: true }} label={t("date","Date")} value={strDate} onChange={(objEvent) => setStrDate(objEvent.target.value)} size="small" /><TextField data-control-id="attendance.daily.department.select" select label={t("department","Department")} value={strDepartment} onChange={(objEvent) => setStrDepartment(objEvent.target.value)} size="small"><MenuItem value="">{t("all","All")}</MenuItem>{lstDepartments.map(([intID,strName]) => <MenuItem key={intID} value={intID ?? ""}>{strName}</MenuItem>)}</TextField><TextField data-control-id="attendance.daily.location.select" select label={t("location","Location")} value={strLocation} onChange={(objEvent) => setStrLocation(objEvent.target.value)} size="small"><MenuItem value="">{t("all","All")}</MenuItem>{lstLocations.map(([intID,strName]) => <MenuItem key={intID} value={intID}>{strName}</MenuItem>)}</TextField><TextField data-control-id="attendance.daily.employee-search.input" placeholder={t("employee_search","Employee Code or Name")} value={strEmployeeSearch} onChange={(objEvent) => setStrEmployeeSearch(objEvent.target.value)} size="small" /><Box className={styles.searchActions}><Button data-control-id="attendance.daily.search.button" className={styles.primaryButton} startIcon={<SearchRoundedIcon />} onClick={() => void searchDaily()}>{t("search","Search")}</Button></Box><Box className={styles.searchActions}><Button data-control-id="attendance.daily.clear.button" className={styles.secondaryButton} startIcon={<ClearRoundedIcon />} onClick={() => void clearDailySearch()}>{t("clear","Clear")}</Button></Box></Box><Alert severity="info" sx={{ mt: 1 }}>{t("cross_midnight_notice","Cross-midnight attendance is outside the current POC and will be rejected.")}</Alert>
        <Stack direction="row" spacing={2} flexWrap="wrap" useFlexGap sx={{ mt: 1, px: 0.5 }}>
          {([
            ...
            ["exceptions", "Exceptions", Object.keys(dicRowErrors).filter((strKey) => dicRowErrors[Number(strKey)]).length],
