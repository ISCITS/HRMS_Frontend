...
export default function AttendancePocPanel({ strView }: AttendancePocPanelProps) {
  ...
  const nodePolicyForm = <Grid component="fieldset" disabled={blnPolicyViewMode} container spacing={1.5} sx={{
    ...
    <Grid item xs={12} lg={6}><Paper variant="outlined" sx={{ p: 1.5, height: "100%" }}><Typography fontWeight={800} sx={{ mb: 1 }}>{t("working_hour_rules", "Working Hour Rules")}</Typography><Grid container spacing={1}>
      {([["decFullDayThresholdHours","full_day_threshold","Full-day Threshold"],["decHalfDayThresholdHours","half_day_threshold","Half-day Threshold"],["decAbsentThresholdHours","absent_threshold","Absent Threshold"]] as const).map(([strName,strKey,strLabel]) => <Grid item xs={12} md={4} key={strName}><Controller name={strName} control={control} render={({ field }) => <TextField {...field} data-control-id={`attendance.policy.${strName}.input`} label={t(strKey,strLabel)} type="number" fullWidth error={!!errors[strName]} inputProps={{ min: 0, step: .25 }} />} /></Grid>)}
      <Grid item xs={12} md={6}><Controller name="strInTime" control={control} render={({ field }) => <TextField {...field} value={field.value ?? ""} data-control-id="attendance.policy.in-time.input" label={t("in_time", "In Time")} type="time" InputLabelProps={{ shrink: true }} fullWidth helperText={t("in_time_help", "Shift start — measures late punch-ins")} />} /></Grid>
      <Grid item xs={12} md={6}><Controller name="strOutTime" control={control} render={({ field }) => <TextField {...field} value={field.value ?? ""} data-control-id="attendance.policy.out-time.input" label={t("out_time", "Out Time")} type="time" InputLabelProps={{ shrink: true }} fullWidth helperText={t("out_time_help", "Shift end — measures early punch-outs")} />} /></Grid>
    ...
    <Grid item xs={12} lg={6}><Paper variant="outlined" sx={{ p: 1.5, height: "100%" }}><Typography fontWeight={800} sx={{ mb: 1 }}>{t("punch_exception_rules", "Punch and Exception Rules")}</Typography><Grid container spacing={1}>
      <Grid item xs={12} md={8}><Grid container spacing={1}>
        <Grid item xs={12} sm={6}><Controller name="intLateGraceMinutes" control={control} render={({ field }) => <TextField {...field} data-control-id="attendance.policy.late-grace.input" label={t("late_grace", "Late Grace (min)")} type="number" fullWidth />} /></Grid>
        <Grid item xs={12} sm={6}><Controller name="intEarlyDepartureGraceMinutes" control={control} render={({ field }) => <TextField {...field} data-control-id="attendance.policy.early-grace.input" label={t("early_grace", "Early Departure Grace")} type="number" fullWidth />} /></Grid>
        <Grid item xs={12} sm={6}><Controller name="strMissingPunchTreatmentCode" control={control} render={({ field }) => <TextField {...field} data-control-id="attendance.policy.missing-punch.select" select label={t("missing_punch", "Missing-punch Treatment")} fullWidth>{([["EXCEPTION","Creates attendance exception"],["ABSENT","Mark as Absent"],["HALF_DAY","Mark as Half Day"],["IGNORE","Ignore"]] as const).map(([strValue, strFallback]) => <MenuItem key={strValue} value={strValue}>{t(`missing_${strValue.toLowerCase()}`, strFallback)}</MenuItem>)}</TextField>} /></Grid>
        <Grid item xs={12} sm={6}><Controller name="intWorkHoursRoundingMinutes" control={control} render={({ field }) => <TextField {...field} data-control-id="attendance.policy.rounding.input" label={t("rounding", "Work-hour Rounding (min)")} type="number" fullWidth />} /></Grid>
      </Grid></Grid>
      <Grid item xs={12} md={4}><Stack spacing={1} justifyContent="space-between" sx={{ height: "100%" }}>
        <Controller name="blnInPunchRequired" control={control} render={({ field }) => <Box sx={{ border: "1px solid #d9e6ef", borderRadius: "9px", px: 1.25, height: 48, display: "flex", alignItems: "center" }}><FormControlLabel control={<Switch data-control-id="attendance.policy.in-required.switch" checked={field.value} onChange={field.onChange} />} label={t("in_required", "IN Required")} sx={{ m: 0 }} /></Box>} />
        <Controller name="blnOutPunchRequired" control={control} render={({ field }) => <Box sx={{ border: "1px solid #d9e6ef", borderRadius: "9px", px: 1.25, height: 48, display: "flex", alignItems: "center" }}><FormControlLabel control={<Switch data-control-id="attendance.policy.out-required.switch" checked={field.value} onChange={field.onChange} />} label={t("out_required", "OUT Required")} sx={{ m: 0 }} /></Box>} />
      </Stack></Grid>
    </Grid></Paper></Grid>
