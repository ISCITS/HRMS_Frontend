...
function getLineSection(dicLine: PayrollResultLineRecord): SalaryStatementLine["strSection"] {
...
function buildSalaryStatementLines(lstDetails: PayrollResultDetailRecord[], lstMonths: MonthColumn[]) {
  const mapLines = new Map<string, SalaryStatementLine>();
  ...
  const dicGrossRow: SalaryStatementLine = { strKey: "summary:gross", strComponent: "Gross Pay", strSection: "summary", intOrder: 20, dicAmounts: dicGross, blnSummary: true };
  const dicDeductionRow: SalaryStatementLine = { strKey: "summary:deduction", strComponent: "Gross Deduction", strSection: "summary", intOrder: 40, dicAmounts: dicDeduction, blnSummary: true };
  const dicNetRow: SalaryStatementLine = { strKey: "summary:net", strComponent: "Net Pay", strSection: "summary", intOrder: 45, dicAmounts: dicNet, blnSummary: true };
...
function getRowTotal(dicLine: SalaryStatementLine, lstMonths: MonthColumn[]) {
...
export default function SalaryStatementReportPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([employeeService.getEmployees(), payrollReportService.getPayrollRegisterRows({})])
      ...
      .catch((objError) => setStrError(objError instanceof Error ? objError.message : "Unable to load salary statement filters."))
      .finally(() => {
  ...
  const lstStatementLines = useMemo(() => buildSalaryStatementLines(lstDetails, lstMonths), [lstDetails, lstMonths]);
  ...
  async function loadReport() {
    ...
    if (!dicSearch.intEmployeeID) {
      setStrError("Select an employee to generate the Salary statement.");
      return;
    }
    ...
    try {
      ...
      setStrError(objError instanceof Error ? objError.message : "Unable to load Salary statement.");
    } finally {
  ...
  const strReportMarkup = useMemo(() => {
    const strRows = lstStatementLines.map((dicLine) => `
    ...
    `;
  }, [dicSearch.intFinancialYearStart, dicSelectedEmployee, lstMonths, lstStatementLines, strFinancialYearLabel]);

  if (blnRightsLoading || blnLoadingMasters) {
    return <BlockingLoader blnOpen strLabel="Loading salary statement..." />;
  }
        ...
        <Box sx={{ flex: "1 1 auto", minHeight: 0, overflow: "auto", pr: 0.5, scrollbarGutter: "stable" }}>
          {blnLoadingReport ? (
            ...
            <Box sx={{ border: "1px solid #4b5563", minWidth: 1280 }}>
              <Typography sx={{ fontSize: 24, fontWeight: 800, textAlign: "center", py: 1 }}>Salary Statement</Typography>
              ...
              <Box component="table" sx={{ borderCollapse: "collapse", width: "100%", "& th, & td": { border: "1px solid #6b7280", fontSize: 13, p: "4px 5px" }, "& th": { fontWeight: 700 }, "& td:not(:first-of-type)": { textAlign: "right" } }}>
                ...
                <tbody>
                  {lstStatementLines.length ? lstStatementLines.map((dicLine) => (
                    ...
                    <tr>
                      <td colSpan={14} style={{ textAlign: "center", padding: 24 }}>Select filters and search to generate the Salary statement.</td>
                    </tr>
                  )}
                </tbody>
              </Box>
