    ...
    <Stack spacing={3}>
      ...
      <Paper sx={{ p: { xs: 2, md: 3 }, borderRadius: 2, border: "1px solid", borderColor: "divider", boxShadow: "0 20px 48px rgba(15, 23, 42, 0.08)", backgroundImage: "linear-gradient(180deg, rgba(255,255,255,0.98) 0%, rgba(248,250,252,0.96) 100%)" }}>
        ...
        <Box sx={{ display: "flex", justifyContent: "space-between", gap: 2, mt: 4 }}>
          <Button controlId="tenant-admin.editor.previous.button" variant="outlined" onClick={handleBack} disabled={intActiveStep === 0 || blnSubmitting}>Previous</Button>
          {intActiveStep < 3 ? (
    ...
    return (
      <Stack spacing={2.25}>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" }, gap: 2 }}>
          <TextField label="Tenant Name *" inputProps={{ "controlId": "tenant-admin.editor.tenant-name.input" }} value={objForm.basic.strTenantName} onChange={(e) => setField("basic.strTenantName", e.target.value)} error={Boolean(dicErrors["basic.strTenantName"])} helperText={dicErrors["basic.strTenantName"]} fullWidth />
          <TextField label="Tenant Code *" inputProps={{ "controlId": "tenant-admin.editor.tenant-code.input" }} value={objForm.basic.strTenantCode} onChange={(e) => setField("basic.strTenantCode", e.target.value.toUpperCase())} error={Boolean(dicErrors["basic.strTenantCode"])} helperText={dicErrors["basic.strTenantCode"] ?? "Use uppercase letters, numbers, hyphen, or underscore."} fullWidth />
  ...
  function renderDatastoreConfiguration() {
    return (
      <Stack spacing={2.25}>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "1fr 1fr" }, gap: 2 }}>
          <TextField select label="Datastore Type *" inputProps={{ "controlId": "tenant-admin.editor.datastore-type.select" }} value={objForm.datastore.strStoreType} onChange={(e) => setField("datastore.strStoreType", e.target.value)} fullWidth>
          ...
          <TextField label="Database Name *" inputProps={{ "controlId": "tenant-admin.editor.database-name.input" }} value={objForm.datastore.strDatabaseName} onChange={(e) => setField("datastore.strDatabaseName", e.target.value)} error={Boolean(dicErrors["datastore.strDatabaseName"])} helperText={dicErrors["datastore.strDatabaseName"]} fullWidth />
