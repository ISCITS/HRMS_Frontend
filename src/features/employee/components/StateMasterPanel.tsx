    ...
    <Box className={styles.page}>
      ...
      <Box className={styles.controlsCard}>
        ...
        <Box className={styles.searchRow}>
          ...
          <TextField select label={dicLabels.searchStatusPlaceholder} value={dicSearchDraft.status} onChange={(objEvent) => setDicSearchDraft((dicPrevious) => ({ ...dicPrevious, status: objEvent.target.value as SearchForm["status"] }))} fullWidth><MenuItem value="All">All</MenuItem><MenuItem value="Active">{dicCommonLabels.statusActive}</MenuItem><MenuItem value="Inactive">{dicCommonLabels.statusInactive}</MenuItem></TextField>
          <Box className={styles.searchActions}><Button className={styles.primaryButton} startIcon={<SearchRoundedIcon />} onClick={() => setDicSearchApplied(dicSearchDraft)} disabled={blnLoading || blnSubmitting}>{dicCommonLabels.search}</Button></Box>
          <Box className={styles.searchActions}><Button className={styles.secondaryButton} startIcon={<ClearRoundedIcon />} onClick={() => { setDicSearchDraft(dicEmptySearch); setDicSearchApplied(dicEmptySearch); }} disabled={blnLoading || blnSubmitting}>{dicCommonLabels.clear}</Button></Box>
        </Box>
        {blnSubmitting ? (
...
type Status="Active"|"Inactive"; type Mode="add"|"edit"|"view"; type StateRecord={id:string;countryId:number|"";countryName:string;code:string;name:string;status:Status}; type SearchForm={code:string;name:string;status:"All"|Status}; type ConfirmDialogState={strTitle:string;strMessage:string;strConfirmLabel:string;fnOnConfirm:()=>Promise<void>}; type ToastState={blnOpen:boolean;strMessage:string;strSeverity:"success"|"error"}; const dicEmptySearch:SearchForm={code:"",name:"",status:"All"}; const lstRowsPerPageOptions=[10,20,50]; const mapStateRecord=(r:StateApiRecord,o:StateFormOptionsApiRecord):StateRecord=>({id:String(r.intID),countryId:r.intCountryID,countryName:o.lstCountries.find(c=>c.intID===r.intCountryID)?.strLabel??"",code:r.strStateCode,name:r.strStateName,status:r.blnIsActive?"Active":"Inactive"});
function downloadCsv(n:string,rows:StateRecord[]){const h=["Country","State Name","State Code","Status"];const l=[h.join(","),...rows.map(r=>[r.countryName,r.name,r.code,r.status].map(v=>`"${String(v).replace(/"/g,'""')}"`).join(","))];const b=new Blob([l.join("\n")],{type:"text/csv;charset=utf-8;"});const u=URL.createObjectURL(b);const a=document.createElement("a");a.href=u;a.download=n;a.click();URL.revokeObjectURL(u);} function exportPdf(t:string,rows:StateRecord[]){const w=window.open("","_blank","width=1200,height=800");if(!w)return;const s=rows.map(r=>`<tr><td>${r.countryName}</td><td>${r.name}</td><td>${r.code}</td><td>${r.status}</td></tr>`).join("");w.document.write(`<html><head><title>${t}</title><style>body{font-family:Arial,sans-serif;padding:24px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #cbd5e1;padding:10px;text-align:left}th{background:#e2e8f0}</style></head><body><h1>${t}</h1><table><thead><tr><th>Country</th><th>State Name</th><th>State Code</th><th>Status</th></tr></thead><tbody>${s}</tbody></table></body></html>`);w.document.close();w.focus();w.print();}
export default function StateMasterPanel(){const objRouter=useRouter();const {t}=useModuleLabels("state");const {blnLoading:blnRightsLoading,strError:strRightsError,canDoAny,canViewAny,isReadOnly}=useModuleActionAccess(["STATE","STATES"]);const [lstStates,setLstStates]=useState<StateRecord[]>([]);const [objFormOptions,setObjFormOptions]=useState<StateFormOptionsApiRecord>({lstLanguages:[],lstCountries:[]});const [strMode,setStrMode]=useState<Mode>("add");const [blnDialogOpen,setBlnDialogOpen]=useState(false);const [strEditingId,setStrEditingId]=useState("");const [dicForm,setDicForm]=useState<StateFormValues>(createInitialStateForm());const [dicErrors,setDicErrors]=useState<Partial<Record<"countryId"|"code"|"name",string>>>({});const [dicTextTranslationLoading,setDicTextTranslationLoading]=useState<Record<string,boolean>>({});const [dicLastTranslatedSourceByRow,setDicLastTranslatedSourceByRow]=useState<Record<string,string>>({});const [dicSearchDraft,setDicSearchDraft]=useState<SearchForm>(dicEmptySearch);const [dicSearchApplied,setDicSearchApplied]=useState<SearchForm>(dicEmptySearch);const [lstSelectedIds,setLstSelectedIds]=useState<string[]>([]);const [blnLoading,setBlnLoading]=useState(true);const [blnSubmitting,setBlnSubmitting]=useState(false);const [intPage,setIntPage]=useState(1);const [intRowsPerPage,setIntRowsPerPage]=useState(10);const [objConfirmDialog,setObjConfirmDialog]=useState<ConfirmDialogState|null>(null);const [objToast,setObjToast]=useState<ToastState>({blnOpen:false,strMessage:"",strSeverity:"success"});const [dicRowLabelsByLanguageID,setDicRowLabelsByLanguageID]=useState<Record<number,Record<string,string>>>({});
const dicCommonLabels={cancel:t("cancel"),clear:t("clear"),close:t("close"),exportExcel:t("export_excel"),exportPdf:t("export_pdf"),save:t("save"),search:t("search"),statusActive:t("status_active"),statusInactive:t("status_inactive"),rowsPerPage:t("rows_per_page"),paginationSeparator:t("pagination_separator"),loading:t("loading"),processing:t("processing")}; const dicLabels={pageTitle:stripMasterTitle(t("page_title")),backButton:t("back_button"),addButton:t("add_button"),dialogAddTitle:t("dialog_add_title"),dialogEditTitle:t("dialog_edit_title"),dialogViewTitle:t("dialog_view_title"),searchNamePlaceholder:t("search_name_placeholder"),searchCodePlaceholder:t("search_code_placeholder"),searchStatusPlaceholder:t("search_status_placeholder"),tableCountry:t("table_country"),tableName:t("table_name"),tableCode:t("table_code"),tableStatus:t("table_status"),tableActions:t("table_actions"),loadingRecords:t("loading_records"),emptyMessage:t("empty_message"),fieldCountry:t("field_country"),fieldName:t("field_name"),fieldCode:t("field_code"),fieldIsActive:t("field_is_active","Is Active"),selectCountry:t("select_country"),saving:t("saving","Saving..."),requestFailed:t("request_failed"),saveSuccess:t("save_success"),updateSuccess:t("update_success"),deleteSuccess:t("delete_success"),activateSuccess:t("activate_success"),deactivateSuccess:t("deactivate_success"),bulkActivateSuccess:t("bulk_activate_success"),bulkDeactivateSuccess:t("bulk_deactivate_success"),bulkDeleteSuccess:t("bulk_delete_success"),bulkRowsSelected:t("bulk_rows_selected"),bulkActivate:t("bulk_activate"),bulkDeactivate:t("bulk_deactivate"),bulkDelete:t("bulk_delete"),confirmButton:t("confirm_button"),confirmDeleteTitle:t("confirm_delete_title"),confirmDeleteMessage:t("confirm_delete_message"),confirmActivateTitle:t("confirm_activate_title"),confirmActivateMessage:t("confirm_activate_message"),confirmDeactivateTitle:t("confirm_deactivate_title"),confirmDeactivateMessage:t("confirm_deactivate_message"),confirmBulkDeleteTitle:t("confirm_bulk_delete_title"),confirmBulkDeleteMessage:t("confirm_bulk_delete_message"),confirmBulkActivateTitle:t("confirm_bulk_activate_title"),confirmBulkActivateMessage:t("confirm_bulk_activate_message"),confirmBulkDeactivateTitle:t("confirm_bulk_deactivate_title"),confirmBulkDeactivateMessage:t("confirm_bulk_deactivate_message"),validationCountryRequired:t("validation_country_required"),validationCodeRequired:t("validation_code_required"),validationCodeFormat:t("validation_code_format"),validationNameRequired:t("validation_name_required"),validationNameMin:t("validation_name_min")};
...
async function loadData(){if(!canViewAny()){setLstStates([]);setLstSelectedIds([]);setBlnLoading(false);return;}setBlnLoading(true);try{const opt=await stateService.getStateFormOptions(intDefaultLanguageID);const res=await masterApiService.getStates();setObjFormOptions(opt);setLstStates(res.Data.map(r=>mapStateRecord(r,opt)));setLstSelectedIds([]);setIntPage(1);}finally{setBlnLoading(false);}} useEffect(()=>{if(!blnRightsLoading){void loadData();}},[blnRightsLoading]); useEffect(()=>{void stateService.getStateFormOptions(intDefaultLanguageID).then(setObjFormOptions).catch(()=>undefined);},[intDefaultLanguageID]);
const buildFixedLanguageRow=(id:number,name:string,code:string,rows:StateTextFormValue[]):StateTextFormValue=>{const lang=objFormOptions.lstLanguages.find(d=>d.intID===id);const ex=rows.find(d=>Number(d.intLanguageID)===id);return {...createEmptyStateTextRow(),...ex,intLanguageID:id,strLanguageName:lang?.strLabel??ex?.strLanguageName??"",strStateName:name,strStateCode:code};}; const ensureTenantLanguageRows=(v:StateFormValues)=>({...v,lstTexts:[buildFixedLanguageRow(intDefaultLanguageID,v.name,v.code,v.lstTexts),buildFixedLanguageRow(intSecondaryLanguageID,v.lstTexts.find(d=>Number(d.intLanguageID)===intSecondaryLanguageID)?.strStateName??"",v.code,v.lstTexts)]}); const syncEnglishName=(name:string)=>setDicForm(p=>{const n=ensureTenantLanguageRows(p);return {...n,lstTexts:n.lstTexts.map((d,i)=>i===0?{...d,strStateName:name}:d)};}); const syncCode=(code:string)=>setDicForm(p=>({...p,lstTexts:p.lstTexts.map(d=>({...d,strStateCode:code}))})); const updateTextRow=(rowId:string,field:keyof StateTextFormValue,value:string|number)=>setDicForm(p=>({...p,lstTexts:p.lstTexts.map(d=>d.strRowID!==rowId?d:field==="intLanguageID"?{...d,intLanguageID:Number(value),strLanguageName:objFormOptions.lstLanguages.find(o=>o.intID===Number(value))?.strLabel??""}:{...d,[field]:value})}));
async function translateTextRow(rowId:string,languageId:number){const lang=objFormOptions.lstLanguages.find(d=>d.intID===languageId);const src=dicForm.name.trim();if(!lang||languageId===intDefaultLanguageID||!src)return;const row=dicForm.lstTexts.find(d=>d.strRowID===rowId);const last=(dicLastTranslatedSourceByRow[rowId]??"").trim();if(row?.strStateName.trim()&&last===src)return;setDicTextTranslationLoading(p=>({...p,[rowId]:true}));try{const tr=await stateService.translateStateText(src,intDefaultLanguageID,languageId);setDicForm(p=>({...p,lstTexts:p.lstTexts.map(d=>d.strRowID===rowId?{...d,intLanguageID:languageId,strLanguageName:lang.strLabel,strStateName:tr}:d)}));setDicLastTranslatedSourceByRow(p=>({...p,[rowId]:src}));}catch(e){showToast(e instanceof Error?e.message:dicLabels.requestFailed,"error");}finally{setDicTextTranslationLoading(p=>({...p,[rowId]:false}));}} const handleTranslateClick=async()=>{const row=dicForm.lstTexts[1];if(!row)return;const id=Number(row.intLanguageID)||intSecondaryLanguageID;if(id&&id!==intDefaultLanguageID)await translateTextRow(row.strRowID,id);};
useEffect(()=>{let mounted=true;const ids=Array.from(new Set(dicForm.lstTexts.map(d=>Number(d.intLanguageID)).filter(id=>Number.isFinite(id)&&id>0)));const load=ids.filter(id=>!dicRowLabelsByLanguageID[id]);if(load.length===0)return ()=>{mounted=false;};void Promise.all(load.map(async id=>({id,labels:(await labelService.getModuleLabels(id,"state")).labels??{}}))).then(res=>{if(!mounted)return;setDicRowLabelsByLanguageID(p=>{const n={...p};for(const r of res)n[r.id]=r.labels;return n;});});return ()=>{mounted=false;};},[dicForm.lstTexts,dicRowLabelsByLanguageID]); const getRowLabel=(id:number|"",k:string,f:string)=>{const n=Number(id);return !Number.isFinite(n)||n<=0?f:dicRowLabelsByLanguageID[n]?.[k]??f;};
const lstFiltered=useMemo(()=>lstStates.filter(r=>(!dicSearchApplied.code||r.code.toLowerCase().includes(dicSearchApplied.code.toLowerCase()))&&(!dicSearchApplied.name||r.name.toLowerCase().includes(dicSearchApplied.name.toLowerCase()))&&(dicSearchApplied.status==="All"||r.status===dicSearchApplied.status)),[dicSearchApplied,lstStates]); const intPageCount=Math.max(1,Math.ceil(lstFiltered.length/intRowsPerPage)); const intCurrentPage=Math.min(intPage,intPageCount); const intStartIndex=(intCurrentPage-1)*intRowsPerPage; const lstVisible=lstFiltered.slice(intStartIndex,intStartIndex+intRowsPerPage); const blnAllVisibleSelected=lstVisible.length>0&&lstVisible.every(r=>lstSelectedIds.includes(r.id)); const blnSomeVisibleSelected=!blnAllVisibleSelected&&lstSelectedIds.some(id=>lstVisible.some(r=>r.id===id));
async function ensureStateFormOptionsLoaded(){if(objFormOptions.lstLanguages.length>0)return objFormOptions;const o=await stateService.getStateFormOptions(intDefaultLanguageID);setObjFormOptions(o);return o;} async function openDialog(nextMode:Mode,row?:StateRecord){const opt=await ensureStateFormOptionsLoaded();setStrMode(nextMode);setStrEditingId(row?.id??"");setDicErrors({});setDicLastTranslatedSourceByRow({});if(!row){setDicForm(ensureTenantLanguageRows(createInitialStateForm()));setBlnDialogOpen(true);return;} const detail=await stateService.getState(Number(row.id),intDefaultLanguageID);setDicForm(ensureTenantLanguageRows(toStateFormValues(detail,opt)));setBlnDialogOpen(true);} const closeDialog=()=>setBlnDialogOpen(false); const showToast=(m:string,s:ToastState["strSeverity"]="success")=>setObjToast({blnOpen:true,strMessage:m,strSeverity:s}); const closeToast=()=>setObjToast(p=>({...p,blnOpen:false})); const openConfirmDialog=(d:ConfirmDialogState)=>setObjConfirmDialog(d); const closeConfirmDialog=()=>setObjConfirmDialog(null);
async function executeConfirmedAction(){if(!objConfirmDialog)return;setBlnSubmitting(true);try{await objConfirmDialog.fnOnConfirm();}catch(e){showToast(e instanceof Error?e.message:dicLabels.requestFailed,"error");}finally{setBlnSubmitting(false);closeConfirmDialog();}} function validateForm(){const e:Partial<Record<"countryId"|"code"|"name",string>>={};const c=dicForm.code.trim().toUpperCase(),n=dicForm.name.trim(); if(dicForm.countryId==="")e.countryId=dicLabels.validationCountryRequired; if(!c)e.code=dicLabels.validationCodeRequired; else if(!/^[A-Z0-9 /_-]{2,20}$/.test(c))e.code=dicLabels.validationCodeFormat; if(!n)e.name=dicLabels.validationNameRequired; else if(n.length<2)e.name=dicLabels.validationNameMin; setDicErrors(e); return Object.keys(e).length===0; }
async function saveState(){if(!validateForm())return;setBlnSubmitting(true);try{if(strMode==="add")await stateService.createState(dicForm);else await stateService.updateState(Number(strEditingId),dicForm);await loadData();closeDialog();showToast(strMode==="add"?dicLabels.saveSuccess:dicLabels.updateSuccess);}catch(e){showToast(e instanceof Error?e.message:dicLabels.requestFailed,"error");}finally{setBlnSubmitting(false);}} const toggleSelection=(id:string)=>setLstSelectedIds(p=>p.includes(id)?p.filter(v=>v!==id):[...p,id]); const toggleSelectAll=()=>blnAllVisibleSelected?setLstSelectedIds(p=>p.filter(id=>!lstVisible.some(r=>r.id===id))):setLstSelectedIds(p=>[...new Set([...p,...lstVisible.map(r=>r.id)])]);
const bulkUpdateStatus=(s:Status)=>openConfirmDialog({strTitle:s==="Active"?dicLabels.confirmBulkActivateTitle:dicLabels.confirmBulkDeactivateTitle,strMessage:(s==="Active"?dicLabels.confirmBulkActivateMessage:dicLabels.confirmBulkDeactivateMessage).replace("{count}",String(lstSelectedIds.length)).replace("{status}",s==="Active"?dicCommonLabels.statusActive.toLowerCase():dicCommonLabels.statusInactive.toLowerCase()),strConfirmLabel:s==="Active"?dicLabels.bulkActivate:dicLabels.bulkDeactivate,fnOnConfirm:async()=>{await masterApiService.bulkStateStatus(lstSelectedIds.map(Number),s==="Active");await loadData();showToast(s==="Active"?dicLabels.bulkActivateSuccess:dicLabels.bulkDeactivateSuccess);}}); const bulkDelete=()=>openConfirmDialog({strTitle:dicLabels.confirmBulkDeleteTitle,strMessage:dicLabels.confirmBulkDeleteMessage.replace("{count}",String(lstSelectedIds.length)),strConfirmLabel:dicLabels.bulkDelete,fnOnConfirm:async()=>{await masterApiService.bulkStateDelete(lstSelectedIds.map(Number));await loadData();showToast(dicLabels.bulkDeleteSuccess);}}); const deleteRecord=(id:string)=>openConfirmDialog({strTitle:dicLabels.confirmDeleteTitle,strMessage:dicLabels.confirmDeleteMessage,strConfirmLabel:t("delete"),fnOnConfirm:async()=>{await masterApiService.bulkStateDelete([Number(id)]);await loadData();showToast(dicLabels.deleteSuccess);}}); const toggleStatus=(id:string)=>{const item=lstStates.find(d=>d.id===id);if(!item)return;const next=item.status==="Active"?"Inactive":"Active";openConfirmDialog({strTitle:next==="Active"?dicLabels.confirmActivateTitle:dicLabels.confirmDeactivateTitle,strMessage:(next==="Active"?dicLabels.confirmActivateMessage:dicLabels.confirmDeactivateMessage).replace("{status}",next==="Active"?dicCommonLabels.statusActive.toLowerCase():dicCommonLabels.statusInactive.toLowerCase()),strConfirmLabel:next==="Active"?t("activate"):t("deactivate"),fnOnConfirm:async()=>{await masterApiService.bulkStateStatus([Number(id)],next==="Active");await loadData();showToast(next==="Active"?dicLabels.activateSuccess:dicLabels.deactivateSuccess);}})};
return <Box className={styles.page}><Box className={styles.topBar}><Button className={styles.backButton} startIcon={<ArrowBackRoundedIcon/>} onClick={()=>objRouter.back()}>{dicLabels.backButton}</Button></Box><Box className={styles.controlsCard}>{strRightsError?<Typography sx={{mt:1,color:"#b45309",fontSize:"0.85rem"}}>{strRightsError}</Typography>:null}{!blnRightsLoading&&blnCanView&&blnReadOnly?<Typography sx={{mt:1,color:"#1d4ed8",fontSize:"0.85rem",fontWeight:700}}>{t("read_only_mode","You have view-only access for State.")}</Typography>:null}<Box className={styles.searchRow}><TextField value={dicSearchDraft.name} onChange={e=>setDicSearchDraft(p=>({...p,name:e.target.value}))} placeholder={dicLabels.searchNamePlaceholder} fullWidth/><TextField value={dicSearchDraft.code} onChange={e=>setDicSearchDraft(p=>({...p,code:e.target.value.toUpperCase()}))} placeholder={dicLabels.searchCodePlaceholder} fullWidth/><TextField select label={dicLabels.searchStatusPlaceholder} value={dicSearchDraft.status} onChange={e=>setDicSearchDraft(p=>({...p,status:e.target.value as SearchForm["status"]}))} fullWidth><MenuItem value="All">All</MenuItem><MenuItem value="Active">{dicCommonLabels.statusActive}</MenuItem><MenuItem value="Inactive">{dicCommonLabels.statusInactive}</MenuItem></TextField><Box className={styles.searchActions}><Button className={styles.primaryButton} startIcon={<SearchRoundedIcon/>} onClick={()=>{setDicSearchApplied(dicSearchDraft);setIntPage(1);}} disabled={blnLoading||blnSubmitting}>{dicCommonLabels.search}</Button></Box><Box className={styles.searchActions}><Button className={styles.secondaryButton} startIcon={<ClearRoundedIcon/>} onClick={()=>{setDicSearchDraft(dicEmptySearch);setDicSearchApplied(dicEmptySearch);setIntPage(1);}} disabled={blnLoading||blnSubmitting}>{dicCommonLabels.clear}</Button></Box></Box>{lstSelectedIds.length>0&&!blnReadOnly&&(blnCanChangeStatus||blnCanDelete)?<Box className={styles.bulkBar}><Typography className={styles.bulkCount}>{lstSelectedIds.length} {dicLabels.bulkRowsSelected}</Typography>{blnCanChangeStatus?<Button className={styles.bulkActivate} onClick={()=>bulkUpdateStatus("Active")} disabled={blnSubmitting}>{dicLabels.bulkActivate}</Button>:null}{blnCanChangeStatus?<Button className={styles.bulkDeactivate} onClick={()=>bulkUpdateStatus("Inactive")} disabled={blnSubmitting}>{dicLabels.bulkDeactivate}</Button>:null}{blnCanDelete?<Button className={styles.bulkDelete} onClick={bulkDelete} disabled={blnSubmitting}>{dicLabels.bulkDelete}</Button>:null}</Box>:null}</Box><Box className={styles.tableCard}><Box sx={{display:"flex",justifyContent:"space-between",alignItems:{xs:"stretch",md:"center"},gap:1.25,flexWrap:"wrap",pb:1}}><Box sx={{display:"flex",gap:1,flexWrap:"wrap"}}>{blnCanAdd?<Button className={styles.primaryButton} startIcon={<AddRoundedIcon/>} onClick={()=>void openDialog("add")} disabled={blnLoading||blnSubmitting||blnRightsLoading}>{dicLabels.addButton}</Button>:null}{blnCanExport?<Button className={styles.secondaryButton} startIcon={<DownloadRoundedIcon/>} onClick={()=>downloadCsv("state-master.csv",lstFiltered)} disabled={blnLoading||blnSubmitting||blnRightsLoading}>{dicCommonLabels.exportExcel}</Button>:null}{blnCanExport?<Button className={styles.secondaryButton} startIcon={<DownloadRoundedIcon/>} onClick={()=>exportPdf(dicLabels.pageTitle,lstFiltered)} disabled={blnLoading||blnSubmitting||blnRightsLoading}>{dicCommonLabels.exportPdf}</Button>:null}</Box>{!blnLoading&&lstFiltered.length>0?<Box className={styles.paginationBar} sx={{p:0,justifyContent:{xs:"flex-start",md:"flex-end"}}}><Box className={styles.paginationInfo}><Typography className={styles.paginationLabel}>{dicCommonLabels.rowsPerPage}</Typography><TextField select size="small" value={String(intRowsPerPage)} onChange={e=>{setIntRowsPerPage(Number(e.target.value));setIntPage(1);}} className={styles.rowsPerPageSelect}>{lstRowsPerPageOptions.map(o=><MenuItem key={o} value={String(o)}>{o}</MenuItem>)}</TextField><Typography className={styles.paginationRange}>{intStartIndex+1}-{Math.min(intStartIndex+intRowsPerPage,lstFiltered.length)} {dicCommonLabels.paginationSeparator} {lstFiltered.length}</Typography></Box><Pagination count={intPageCount} page={intCurrentPage} onChange={(_,p)=>setIntPage(p)} size="small" color="primary" showFirstButton showLastButton/></Box>:null}</Box>{blnRightsLoading||blnLoading?<Box className={styles.emptyState}><CircularProgress size={24}/><Typography sx={{mt:1}}>{dicLabels.loadingRecords}</Typography></Box>:!blnCanView?<Box className={styles.emptyState}><Typography sx={{fontWeight:800,color:"#0f172a"}}>State access is not available for your user group.</Typography></Box>:<Box className={styles.tableWrap}><table className={styles.table}><thead><tr><th><Checkbox checked={blnAllVisibleSelected} indeterminate={blnSomeVisibleSelected} onChange={toggleSelectAll}/></th><th>{dicLabels.tableActions}</th><th>{dicLabels.tableCountry}</th><th>{dicLabels.tableName}</th><th>{dicLabels.tableCode}</th><th>{dicLabels.tableStatus}</th></tr></thead><tbody>{lstFiltered.length===0?<tr><td className={styles.emptyState} colSpan={6}>{dicLabels.emptyMessage}</td></tr>:lstVisible.map(r=>{const s=lstSelectedIds.includes(r.id);return <tr key={r.id} className={s?styles.selectedRow:undefined}><td><Checkbox checked={s} onChange={()=>toggleSelection(r.id)}/></td><td><CommonRowActions blnCanView={blnCanView} blnCanEdit={blnCanEdit} blnCanDelete={blnCanDelete} blnCanToggle={blnCanChangeStatus} blnToggleActive={r.status==="Active"} onView={()=>void openDialog("view",r)} onEdit={()=>void openDialog("edit",r)} onDelete={()=>deleteRecord(r.id)} onToggle={()=>toggleStatus(r.id)}/></td><td>{r.countryName||"-"}</td><td>{r.name}</td><td>{r.code}</td><td><span className={`${styles.statusPill} ${r.status==="Active"?styles.statusActive:styles.statusInactive}`}>{r.status==="Active"?dicCommonLabels.statusActive:dicCommonLabels.statusInactive}</span></td></tr>;})}</tbody></table></Box>}</Box><CommonMasterDialog blnOpen={blnDialogOpen} onClose={closeDialog} strTitle={strMode==="add"?dicLabels.dialogAddTitle:strMode==="edit"?dicLabels.dialogEditTitle:dicLabels.dialogViewTitle} strSecondaryLabel={strMode==="view"?dicCommonLabels.close:dicCommonLabels.cancel} strPrimaryLabel={blnSubmitting?dicLabels.saving:dicCommonLabels.save} onPrimaryAction={saveState} blnPrimaryDisabled={blnSubmitting} blnHidePrimary={strMode==="view"} paperClassName={styles.dialogPaper} maxWidth="xl" paperSx={{width:"min(1220px, calc(100vw - 44px))",overflow:"hidden"}} contentSx={{overflowX:"hidden",overflowY:"visible"}} nodeContent={<Box sx={{display:"grid",gap:2,pt:0.5}}><Box sx={{display:"grid",gap:1.6,gridTemplateColumns:{xs:"1fr",md:"repeat(3, minmax(0, 1fr))"},alignItems:"start"}}><TextField select label={`${dicLabels.fieldCountry} *`} value={dicForm.countryId===""?"":String(dicForm.countryId)} disabled={strMode==="view"} onChange={e=>{setDicErrors(p=>({...p,countryId:undefined}));setDicForm(p=>({...p,countryId:e.target.value?Number(e.target.value):""}));}} error={Boolean(dicErrors.countryId)} helperText={dicErrors.countryId} fullWidth><MenuItem value="">{dicLabels.selectCountry}</MenuItem>{objFormOptions.lstCountries.map(c=><MenuItem key={c.intID} value={String(c.intID)}>{c.strLabel}{c.strCode?` (${c.strCode})`:""}</MenuItem>)}</TextField><TextField label={`${dicLabels.fieldName} *`} value={dicForm.name} disabled={strMode==="view"} onChange={e=>{const v=e.target.value;setDicErrors(p=>({...p,name:undefined}));setDicForm(p=>({...p,name:v}));syncEnglishName(v);}} error={Boolean(dicErrors.name)} helperText={dicErrors.name} fullWidth/><TextField label={`${dicLabels.fieldCode} *`} value={dicForm.code} disabled={strMode==="view"} onChange={e=>{const v=e.target.value.toUpperCase();setDicErrors(p=>({...p,code:undefined}));setDicForm(p=>({...p,code:v}));syncCode(v);}} error={Boolean(dicErrors.code)} helperText={dicErrors.code} fullWidth/></Box><Box sx={{display:"flex",justifyContent:"space-between",alignItems:{xs:"flex-start",md:"center"},gap:1.25,flexWrap:"wrap"}}><Box><Typography sx={{fontWeight:800,color:"#0f172a"}}>{t("multilingual_text","Multilingual Text")}</Typography><Typography sx={{color:"#64748b",fontSize:"0.86rem",mt:0.25}}>{t("multilingual_text_help","Add translated state names for supported languages.")}</Typography></Box><Box sx={{display:"flex",gap:1.1,alignItems:"center",ml:"auto"}}><Button variant="outlined" startIcon={<AddRoundedIcon/>} disabled>{t("add_language","Add Language")}</Button><Button variant="contained" onClick={()=>void handleTranslateClick()} disabled={strMode==="view"||blnSubmitting||dicTextTranslationLoading[dicForm.lstTexts[1]?.strRowID??""]} sx={{minWidth:108,borderRadius:"12px",background:"#2563eb",boxShadow:"none","&:hover":{background:"#1d4ed8",boxShadow:"none"}}}>{dicTextTranslationLoading[dicForm.lstTexts[1]?.strRowID??""]?<CircularProgress size={18} sx={{color:"#ffffff"}}/>:t("translate","AI Translate")}</Button></Box></Box><Box sx={{display:"grid",gap:1.2}}>{dicForm.lstTexts.map((d,i)=><Box key={d.strRowID} sx={{display:"grid",gap:1.2,gridTemplateColumns:{xs:"1fr",md:"minmax(0, 0.95fr) minmax(0, 1.35fr) minmax(0, 0.95fr)"},alignItems:"start",border:"1px solid rgba(203,213,225,0.8)",borderRadius:"16px",p:1.2,background:"#f8fafc"}}><TextField select label={getRowLabel(d.intLanguageID,"language",t("language","Language"))} value={d.intLanguageID} InputLabelProps={{shrink:true}} SelectProps={{displayEmpty:true,renderValue:v=>objFormOptions.lstLanguages.find(l=>l.intID===Number(v))?.strLabel??d.strLanguageName??""}} disabled fullWidth>{objFormOptions.lstLanguages.map(l=><MenuItem key={l.intID} value={l.intID}>{l.strLabel}</MenuItem>)}</TextField><TextField label={getRowLabel(d.intLanguageID,"field_name",dicLabels.fieldName)} value={d.strStateName} onChange={e=>{const v=e.target.value;updateTextRow(d.strRowID,"strStateName",v);if(i===0){setDicErrors(p=>({...p,name:undefined}));setDicForm(p=>({...p,name:v}));}}} disabled={strMode==="view"||i===0} InputProps={{endAdornment:dicTextTranslationLoading[d.strRowID]?<InputAdornment position="end"><CircularProgress size={18} sx={{color:"#2563eb"}}/></InputAdornment>:undefined}} fullWidth/><TextField label={getRowLabel(d.intLanguageID,"field_code",dicLabels.fieldCode)} value={d.strStateCode} disabled fullWidth/></Box>)}</Box><Box className={styles.switchRow}><Typography className={styles.switchLabel}>{dicLabels.fieldIsActive}</Typography><Switch checked={dicForm.status==="Active"} disabled={strMode==="view"} onChange={(_,c)=>setDicForm(p=>({...p,status:c?"Active":"Inactive"}))}/></Box></Box>}/><CommonConfirmDialog blnOpen={Boolean(objConfirmDialog)} strTitle={objConfirmDialog?.strTitle} strMessage={objConfirmDialog?.strMessage} strCancelLabel={dicCommonLabels.cancel} strConfirmLabel={objConfirmDialog?.strConfirmLabel??dicLabels.confirmButton} blnConfirmDisabled={blnSubmitting} onClose={closeConfirmDialog} onConfirm={executeConfirmedAction}/><BlockingLoader blnOpen={blnLoading||blnRightsLoading||blnSubmitting} strLabel={blnLoading||blnRightsLoading?dicCommonLabels.loading:dicCommonLabels.processing} intZIndex={1400}/><Snackbar open={objToast.blnOpen} autoHideDuration={3500} onClose={closeToast} anchorOrigin={{vertical:"top",horizontal:"right"}}><Alert severity={objToast.strSeverity} onClose={closeToast} variant="filled" sx={{width:"100%"}}>{objToast.strMessage}</Alert></Snackbar></Box>; }
