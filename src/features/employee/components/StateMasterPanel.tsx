...
export default function StateMasterPanel() {
  ...
  const dicModuleLabels = {
    ...
    sectionBasicInformation: t("section_basic_information"),
  ...
  function validateForm() {
    ...
    if (dicForm.countryId === "") { dicNextErrors.countryId = dicModuleLabels.validationCountryRequired; }
    if (!strCode) { dicNextErrors.code = dicModuleLabels.validationCodeRequired; } else if (!/^[A-Z0-9 /_-]{2,20}$/.test(strCode)) { dicNextErrors.code = dicModuleLabels.validationCodeFormat; }
  ...
  function saveState() {
    ...
    setBlnSubmitting(true);
    objRequest.then(() => loadData()).then(() => { closeDialog(); showToast(strMode === "add" ? dicModuleLabels.saveSuccess : dicModuleLabels.updateSuccess); }).catch((objError) => showToast(objError instanceof Error ? objError.message : dicModuleLabels.requestFailed, "error")).finally(() => setBlnSubmitting(false));
  }
  ...
  function deleteRecord(strId: string) {
    openConfirmDialog({
      ...
      strConfirmLabel: dicCommonLabels.cancel === "Delete" ? dicCommonLabels.cancel : t("delete"),
      fnOnConfirm: async () => {
  ...
  function toggleStatus(strId: string) {
    const objItem = lstStates.find((dicItem) => dicItem.id === strId);
    if (!objItem) { return; }
  ...
  return (
    <Box className={styles.page}>
      ...
      <Box className={styles.controlsCard}>
        ...
        <Box className={styles.searchRow}><TextField value={dicSearchDraft.name} onChange={(objEvent) => setDicSearchDraft((dicPrevious) => ({ ...dicPrevious, name: objEvent.target.value }))} placeholder={dicModuleLabels.searchNamePlaceholder} fullWidth /><TextField value={dicSearchDraft.code} onChange={(objEvent) => setDicSearchDraft((dicPrevious) => ({ ...dicPrevious, code: objEvent.target.value.toUpperCase() }))} placeholder={dicModuleLabels.searchCodePlaceholder} fullWidth /><TextField select label={dicModuleLabels.searchStatusPlaceholder} value={dicSearchDraft.status} onChange={(objEvent) => setDicSearchDraft((dicPrevious) => ({ ...dicPrevious, status: objEvent.target.value as SearchForm["status"] }))} fullWidth><MenuItem value="All">All</MenuItem><MenuItem value="Active">{dicCommonLabels.statusActive}</MenuItem><MenuItem value="Inactive">{dicCommonLabels.statusInactive}</MenuItem></TextField><Box className={styles.searchActions}><Button className={styles.primaryButton} startIcon={<SearchRoundedIcon />} onClick={() => { setDicSearchApplied(dicSearchDraft); setIntPage(1); }} disabled={blnLoading || blnSubmitting}>{dicCommonLabels.search}</Button></Box><Box className={styles.searchActions}><Button className={styles.secondaryButton} startIcon={<ClearRoundedIcon />} onClick={() => { setDicSearchDraft(dicEmptySearch); setDicSearchApplied(dicEmptySearch); setIntPage(1); }} disabled={blnLoading || blnSubmitting}>{dicCommonLabels.clear}</Button></Box></Box>
        {lstSelectedIds.length > 0 && (blnCanEdit || blnCanDelete) ? <Box className={styles.bulkBar}><Typography className={styles.bulkCount}>{lstSelectedIds.length} {dicModuleLabels.bulkRowsSelected}</Typography>{blnCanEdit ? <Button className={styles.bulkActivate} onClick={() => bulkUpdateStatus("Active")} disabled={blnSubmitting}>{dicModuleLabels.bulkActivate}</Button> : null}{blnCanEdit ? <Button className={styles.bulkDeactivate} onClick={() => bulkUpdateStatus("Inactive")} disabled={blnSubmitting}>{dicModuleLabels.bulkDeactivate}</Button> : null}{blnCanDelete ? <Button className={styles.bulkDelete} onClick={bulkDelete} disabled={blnSubmitting}>{dicModuleLabels.bulkDelete}</Button> : null}</Box> : null}
      </Box>
      <Box className={styles.tableCard}>
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: { xs: "stretch", md: "center" }, gap: 1.25, flexWrap: "wrap", pb: 1 }}><Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>{blnCanAdd ? <Button className={styles.primaryButton} startIcon={<AddRoundedIcon />} onClick={() => openDialog("add")} disabled={blnLoading || blnSubmitting || blnRightsLoading}>{dicModuleLabels.addButton}</Button> : null}{blnCanExport ? <Button className={styles.secondaryButton} startIcon={<DownloadRoundedIcon />} onClick={() => downloadCsv("state-master.xls", lstFiltered, dicCountries)} disabled={blnLoading || blnSubmitting || blnRightsLoading}>{dicCommonLabels.exportExcel}</Button> : null}{blnCanExport ? <Button className={styles.secondaryButton} startIcon={<DownloadRoundedIcon />} onClick={() => exportPdf(dicModuleLabels.pageTitle, lstFiltered, dicCountries)} disabled={blnLoading || blnSubmitting || blnRightsLoading}>{dicCommonLabels.exportPdf}</Button> : null}</Box>{!blnLoading && lstFiltered.length > 0 ? <Box className={styles.paginationBar} sx={{ p: 0, justifyContent: { xs: "flex-start", md: "flex-end" } }}><Box className={styles.paginationInfo}><Typography className={styles.paginationLabel}>{dicCommonLabels.rowsPerPage}</Typography><TextField select size="small" value={String(intRowsPerPage)} onChange={(objEvent) => { setIntRowsPerPage(Number(objEvent.target.value)); setIntPage(1); }} className={styles.rowsPerPageSelect}>{lstRowsPerPageOptions.map((intOption) => <MenuItem key={intOption} value={String(intOption)}>{intOption}</MenuItem>)}</TextField><Typography className={styles.paginationRange}>{intStartIndex + 1}-{Math.min(intStartIndex + intRowsPerPage, lstFiltered.length)} {dicCommonLabels.paginationSeparator} {lstFiltered.length}</Typography></Box><Pagination count={intPageCount} page={intCurrentPage} onChange={(_, intNextPage) => setIntPage(intNextPage)} size="small" color="primary" showFirstButton showLastButton /></Box> : null}</Box>
        {blnLoading ? <Box className={styles.emptyState}><CircularProgress size={24} /><Typography sx={{ mt: 1 }}>{dicModuleLabels.loadingRecords}</Typography></Box> : !blnCanView ? <Box className={styles.emptyState}><Typography>You do not have permission to view this screen.</Typography></Box> : <Box className={styles.tableWrap}><table className={styles.table}><thead><tr><th><Checkbox checked={blnAllVisibleSelected} indeterminate={blnSomeVisibleSelected} onChange={toggleSelectAll} /></th><th>{dicModuleLabels.tableActions}</th><th>{dicModuleLabels.tableCountry}</th><th>{dicModuleLabels.tableName}</th><th>{dicModuleLabels.tableCode}</th><th>{dicModuleLabels.tableStatus}</th></tr></thead><tbody>{lstFiltered.length === 0 ? <tr><td className={styles.emptyState} colSpan={6}>{dicModuleLabels.emptyMessage}</td></tr> : lstVisible.map((dicState) => { const blnSelected = lstSelectedIds.includes(dicState.id); return <tr key={dicState.id} className={blnSelected ? styles.selectedRow : undefined}><td><Checkbox checked={blnSelected} onChange={() => toggleSelection(dicState.id)} /></td><td><Box className={styles.actionCell}><button className={`${styles.iconButton} ${styles.viewIcon}`} type="button" onClick={() => openDialog("view", dicState)}><VisibilityRoundedIcon fontSize="small" /></button>{blnCanEdit ? <button className={`${styles.iconButton} ${styles.editIcon}`} type="button" onClick={() => openDialog("edit", dicState)}><EditRoundedIcon fontSize="small" /></button> : null}{blnCanDelete ? <button className={`${styles.iconButton} ${styles.deleteIcon}`} type="button" onClick={() => deleteRecord(dicState.id)}><DeleteRoundedIcon fontSize="small" /></button> : null}{blnCanChangeStatus ? <button className={`${styles.iconButton} ${styles.toggleIcon}`} type="button" onClick={() => toggleStatus(dicState.id)}><ToggleOnRoundedIcon fontSize="small" /></button> : null}</Box></td><td>{dicCountries[dicState.countryId] ?? "-"}</td><td>{dicState.name}</td><td>{dicState.code}</td><td><span className={`${styles.statusPill} ${dicState.status === "Active" ? styles.statusActive : styles.statusInactive}`}>{dicState.status === "Active" ? dicCommonLabels.statusActive : dicCommonLabels.statusInactive}</span></td></tr>; })}</tbody></table></Box>}
      </Box>
      <Dialog open={blnDialogOpen} onClose={closeDialog} fullWidth maxWidth="sm" PaperProps={{ className: styles.compactDialogPaper }}><DialogTitle>{strMode === "add" ? dicModuleLabels.dialogAddTitle : strMode === "edit" ? dicModuleLabels.dialogEditTitle : dicModuleLabels.dialogViewTitle}</DialogTitle><DialogContent dividers><Box sx={{ display: "grid", gap: 2.25, pt: 1 }}><TextField select label={`${dicModuleLabels.fieldCountry} *`} value={String(dicForm.countryId)} disabled={strMode === "view"} onChange={(objEvent) => setFormField("countryId", objEvent.target.value ? Number(objEvent.target.value) : "")} error={Boolean(dicErrors.countryId)} helperText={dicErrors.countryId} fullWidth><MenuItem value="">{dicModuleLabels.selectCountry}</MenuItem>{lstCountries.map((dicCountry) => <MenuItem key={dicCountry.intID} value={String(dicCountry.intID)}>{dicCountry.strCountryName}</MenuItem>)}</TextField><TextField label={`${dicModuleLabels.fieldName} *`} value={dicForm.name} disabled={strMode === "view"} onChange={(objEvent) => setFormField("name", objEvent.target.value)} error={Boolean(dicErrors.name)} helperText={dicErrors.name} fullWidth /><TextField label={`${dicModuleLabels.fieldCode} *`} value={dicForm.code} disabled={strMode === "view"} onChange={(objEvent) => setFormField("code", objEvent.target.value.toUpperCase())} error={Boolean(dicErrors.code)} helperText={dicErrors.code} fullWidth /><TextField select label={dicModuleLabels.fieldStatus} value={dicForm.status} disabled={strMode === "view"} onChange={(objEvent) => setFormField("status", objEvent.target.value as Status)} fullWidth><MenuItem value="Active">{dicCommonLabels.statusActive}</MenuItem><MenuItem value="Inactive">{dicCommonLabels.statusInactive}</MenuItem></TextField></Box></DialogContent><DialogActions sx={{ px: 3, py: 2 }}><Button className={styles.secondaryButton} onClick={closeDialog}>{strMode === "view" ? dicModuleLabels.close : dicCommonLabels.cancel}</Button>{strMode !== "view" ? <Button className={styles.primaryButton} onClick={saveState} disabled={blnSubmitting}>{blnSubmitting ? dicCommonLabels.processing : strMode === "add" ? t("save") : t("update")}</Button> : null}</DialogActions></Dialog>
