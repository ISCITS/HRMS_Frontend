...
export function usePayrollProcessLogLabels() {
  ...
  const strFallbackError = "Unable to load payroll process log labels.";
  ...
  useEffect(() => {
    ...
    async function loadLabels() {
      ...
      setStrLabelError("");
      try {
        const objResponse = await labelService.getPayrollProcessLogLabels(intLanguageID);
        if (!blnMounted) {
          return;
        }
  ...
  function t(strKey: string, strFallback?: string) {
    ...
    return `[[payroll-process-logs.${strKey}]]`;
  }
