...
function isFlexiComponentEligibilityActive(dicOption?: SalaryStructureFormOptions["lstSalaryComponents"][number]) {
  if (!dicOption) {
    return false;
  }
  if (typeof dicOption.blnIsFlexiComponentEligible === "boolean") {
    return dicOption.blnIsFlexiComponentEligible;
  }
  return isFlexiEligibleComponent(dicOption);
}
...
export default function SalaryStructureEditorPage({
  ...
  }, [dicComponentByID, lstFlexiBasketLines]);
      ...
      const lstUpdatedComponents: SalaryStructureLineFormValue[] = dicPrevious.lstComponents.map((dicLine) => {
        ...
        if (strField === "intSalaryComponentID") {
          ...
          return {
            ...
            strFlexiComponentRole: blnIsFlexiBasket ? "basket" : "normal",
  ...
  function validateFlexiMappings() {
    for (const dicLine of dicForm.lstComponents.filter(isFlexiBasketLine)) {
      ...
      for (const dicMapping of dicLine.lstFlexiMappings) {
        ...
        if (!dicComponent || !isFlexiEligibleComponent(dicComponent)) {
          return t("flexi_component_not_eligible", "Allowed Flexi Options must be configured in component eligibility.");
        }
        if (!dicMapping.blnIsActive) {
          continue;
        }
              ...
              <tbody>
                {dicForm.lstComponents.map((dicLine) => {
                  ...
                  <tr key={dicLine.strRowID}>
                    ...
                    <td>
                      <Typography sx={{ width: 130, fontSize: "0.8rem", fontWeight: 700, color: getFlexiRoleForComponent(dicComponentByID.get(Number(dicLine.intSalaryComponentID))) === "Flexi Bucket" ? "#0f766e" : "#64748b" }}>
                        {getFlexiRoleForComponent(dicComponentByID.get(Number(dicLine.intSalaryComponentID)))}
                      </Typography>
                    </td>
                    <td>
                      <TextField
                        ...
                        <Box component="table" sx={{ borderCollapse: "collapse", minWidth: 760, width: "100%" }}>
                          <Box component="thead" sx={{ "& th": { borderBottom: "1px solid #d9e6ef", color: "#0f172a", fontSize: "0.77rem", fontWeight: 700, px: 2, py: 1.2, textAlign: "left", whiteSpace: "nowrap" } }}>
                            <tr>
                              ...
                              <th>{t("status", "Eligibility Status")}</th>
                              <th>{t("action", "Action")}</th>
                            </tr>
                          </Box>
                              ...
                              <tr key={dicMapping.strRowID}>
                                  ...
                                  <Stack direction="row" spacing={1.15} alignItems="center">
                                    ...
                                    >
                                      ...
                                      {lstFlexiEligibleComponents.map((dicOption) => (
                                        <MenuItem key={dicOption.intID} value={dicOption.intID} disabled={!isFlexiComponentEligibilityActive(dicOption)} data-testid={`salary-structures.editor.flexi-mapping.component.${normalizeSelectToken(dicOption.strCode || dicOption.strLabel)}.option`}>
                                          {dicOption.strLabel}
                                        </MenuItem>
                                      ))}
                                    </TextField>
                                ...
                                <td>
                                  <Box sx={{ background: dicMapping.blnIsActive ? "#dcf8e8" : "#dbeafe", borderRadius: "4px", color: dicMapping.blnIsActive ? "#067647" : "#0757b8", display: "inline-flex", fontSize: "0.75rem", fontWeight: 700, px: 1, py: 0.45 }}>
                                    {dicMapping.blnIsActive ? t("eligible", "Eligible") : t("not_eligible", "Not Eligible")}
                                  </Box>
                                </td>
                                <td>
