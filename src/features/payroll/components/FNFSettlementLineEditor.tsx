...
export default function FNFSettlementLineEditor({ blnOpen, objLine, onClose, onSave }: { blnOpen: boolean; objLine?: FNFSettlementLineRecord | null; onClose: () => void; onSave: (dicValues: FNFLineFormValues) => Promise<void> }) {
  ...
  useEffect(() => {
    setDicForm(objLine ? { intID: objLine.intID, strLineType: objLine.strLineType, strLineCode: objLine.strLineCode, strLineName: objLine.strLineName, decAmount: String(objLine.decAmount ?? 0), blnIsManualOverride: true, strOverrideReason: objLine.strOverrideReason || "", strRemarks: objLine.strRemarks || "" } : dicEmptyLine);
    setStrError("");
  }, [objLine, blnOpen]);
      ...
      <DialogContent>
        <Stack spacing={2} sx={{ pt: 1 }}>
          ...
          <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
            ...
            <TextField required label="Amount" type="number" value={dicForm.decAmount} onChange={(e) => setDicForm((d) => ({ ...d, decAmount: e.target.value }))} fullWidth />
          </Stack>
