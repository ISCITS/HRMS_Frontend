      ...
      <Box className={styles.controlsCard}>
        ...
        <Box sx={{ display: "grid", gap: 1, gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", mt: 1 }}>
          <Autocomplete
            ...
            renderInput={(params) => <TextField {...params} label="Employee Code" inputProps={{ ...params.inputProps, "data-testid": "payroll.fnf-settlements.employee-code.input" }} data-testid="payroll.fnf-settlements.employee-code.input" />}
          />
          ...
          <TextField size="small" select label="Payable / Recoverable" value={dicFilters.payable_type} onChange={(e) => setDicFilters((d) => ({ ...d, payable_type: e.target.value }))} data-testid="payroll.fnf-settlements.payable-type.select">{["All", "payable", "recoverable"].map((s) => <MenuItem key={s} value={s}>{s}</MenuItem>)}</TextField>
          ...
          <Box sx={{ display: "flex", gap: 1, alignItems: "center" }}>
            ...
            <Button className={styles.secondaryButton} size="small" startIcon={<ClearRoundedIcon />} onClick={() => setDicFilters({ employee_code: "", department: "", settlement_month: "", status: "All", exit_type: "", lwd_from: "", lwd_to: "", payable_type: "All" })} data-testid="payroll.fnf-settlements.clear.button">Clear</Button>
          </Box>
        </Box>
      </Box>
