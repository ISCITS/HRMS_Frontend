...
type ConfirmState =
  | { strKind: "submit" }
  | { strKind: "withdraw"; intApplicationID: number }
  | { strKind: "request_withdrawal"; intApplicationID: number }
  | null;
...
const lstStatusOptions = ["all", "draft", "pending", "approved", "withdrawal_pending", "rejected", "withdrawn", "cancelled"];
...
function fnBuildPayload(objValues: LeaveFormValues): LeaveApplyRequest {
  ...
  const blnFromHalf = objValues.strFromDuration === "half";
  const blnToHalf = !blnSingleDay && objValues.strToDuration === "half";
  return {
...
export default function EssLeaveApplicationPanel() {
  ...
  const {
    ...
    fnWithdraw, fnRequestWithdrawal, fnDeleteAttachment: fnDeleteAttachmentRequest,
  ...
  async function fnRequestWithdrawalConfirmed() {
    if (!objConfirm || objConfirm.strKind !== "request_withdrawal" || !strWithdrawReason.trim()) return;
    setBlnSaving(true);
    try {
      await fnRequestWithdrawal(objConfirm.intApplicationID, strWithdrawReason.trim());
  ...
  return <Stack spacing={2}>
    ...
    {blnLoading ? <LoadingSkeleton /> : strLoadError ? <Paper sx={{ p: 3, borderRadius: "18px", border: "1px solid #fecaca", textAlign: "center" }}><Alert severity="error" sx={{ mb: 2 }}>{strLoadError}</Alert><Button startIcon={<RefreshRoundedIcon />} variant="outlined" onClick={() => void fnLoadAll()}>{t("retry", "Retry")}</Button></Paper> : <>
      <Paper id="leave-applications" sx={{ borderRadius: "18px", border: "1px solid #e2e8f0", overflow: "hidden" }}>
        ...
        <Divider />
        {lstPagedApplications.length === 0 ? <EmptyState strMessage={t("applications_empty", "No leave applications match the selected filters.")} /> : blnMobile ? <Stack spacing={1} sx={{ p: 1.5 }}>{lstPagedApplications.map((objApplication) => <ApplicationCard key={objApplication.intID} objApplication={objApplication} blnCanManage={blnCanManage} fnOnView={() => void fnOpenDetail(objApplication.intID)} fnOnEdit={() => void fnOpenEditForm(objApplication)} fnOnWithdraw={() => { setStrWithdrawReason(""); setObjConfirm({ strKind: "withdraw", intApplicationID: objApplication.intID }); }} fnOnRequestWithdrawal={() => { setStrWithdrawReason(""); setObjConfirm({ strKind: "request_withdrawal", intApplicationID: objApplication.intID }); }} />)}</Stack> : <ApplicationTable lstApplications={lstPagedApplications} blnCanManage={blnCanManage} fnOnView={(intApplicationID) => void fnOpenDetail(intApplicationID)} fnOnEdit={(objApplication) => void fnOpenEditForm(objApplication)} fnOnWithdraw={(intApplicationID) => { setStrWithdrawReason(""); setObjConfirm({ strKind: "withdraw", intApplicationID }); }} fnOnRequestWithdrawal={(intApplicationID) => { setStrWithdrawReason(""); setObjConfirm({ strKind: "request_withdrawal", intApplicationID }); }} fnLabel={t} />}
    ...
    <CommonConfirmDialog blnOpen={objConfirm?.strKind === "request_withdrawal"} strTitle={t("confirm_request_withdrawal_title", "Withdraw Approved Leave?")} nodeMessage={<Stack spacing={1}><Typography sx={{ fontSize: ".85rem", color: "#475569" }}>{t("request_withdrawal_hint", "This will send a withdrawal request back through the same approval chain. The leave stays approved until the withdrawal is approved.")}</Typography><TextField autoFocus fullWidth multiline minRows={2} label={t("withdraw_reason", "Withdrawal Reason")} value={strWithdrawReason} onChange={(objEvent) => setStrWithdrawReason(objEvent.target.value)} error={!strWithdrawReason.trim()} helperText={!strWithdrawReason.trim() ? t("withdraw_reason_required", "A reason is required.") : ""} /></Stack>} strCancelLabel={t("cancel", "Cancel")} strConfirmLabel={t("request_withdrawal", "Request Withdrawal")} blnConfirmDisabled={blnSaving || !strWithdrawReason.trim()} blnCancelDisabled={blnSaving} onClose={() => setObjConfirm(null)} onConfirm={() => void fnRequestWithdrawalConfirmed()} />
